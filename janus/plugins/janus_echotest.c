...
typedef struct janus_echotest_session {
	time_t first_timestamp_seconds;
	long first_timestamp_ms;
	time_t first_rtp_seconds;
	long first_rtp_ms;
	int64_t timedelta;
	gboolean rtcp_arrived;
	janus_plugin_session *handle;
	gboolean has_audio;
	gboolean has_video;
	gboolean has_data;
...
void janus_echotest_create_session(janus_plugin_session *handle, int *error) {
	...
	session->first_timestamp_seconds = 0;
	session->first_timestamp_ms = 0;
	session->first_rtp_seconds = 0;
	session->first_rtp_ms = 0;
	session->rtcp_arrived = FALSE;
...
void janus_echotest_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(gateway) {
		...
		if(video && session->video_active && session->ssrc[0] != 0) {
			if (session->first_rtp_seconds == 0) {
				session->first_rtp_seconds = s;
				session->first_rtp_ms = ms;
				JANUS_LOG(LOG_ERR, "First RTP: %d" SCNu32 ".%d", session->first_rtp_seconds, session->first_rtp_ms);
			}
			...
			if((!video && session->audio_active) || (video && session->video_active)) {
				if (session->first_rtp_seconds == 0) {
					session->first_rtp_seconds = s;
					session->first_rtp_ms = ms;
					JANUS_LOG(LOG_ERR, "First RTP: %" SCNu32 ".%d \n", session->first_rtp_seconds, session->first_rtp_ms);
				}
				/* Save the frame if we're recording */
...
void janus_echotest_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	u_int32_t current_time_int = (u_int32_t) current_time;
	...
	if(gateway) {
		...
		if(session->destroyed)
			return;
		if (session->first_timestamp_seconds == 0){
			session->first_timestamp_seconds = 1;
			JANUS_LOG(LOG_ERR, "First RTCP: %" SCNu32 ".%d \n", s, ms);
			u_int32_t seconds_since_first_rtp = s - session->first_rtp_seconds;
			long ms_since_first_rtp = ms - session->first_rtp_ms;
			if (ms_since_first_rtp < 0) {
				ms_since_first_rtp = 1000 + ms_since_first_rtp;
				seconds_since_first_rtp -= 1;
			}
			JANUS_LOG(LOG_ERR, "RTP arrived: %" SCNu32 ".%d \n", seconds_since_first_rtp, ms_since_first_rtp);


			u_int32_t initial_time_unix = 2208988800; //1st January 1970

			/* Variables to hold most/least significant NTP words and sender's ssrc */
			u_int32_t msw = 0;
			int64_t td_seconds = 0;
			int64_t td_miliseconds = 0;
			u_int32_t lsw = 0;
			u_int32_t delay = 0;
			u_int32_t ssrc = 0;

			long miliseconds;
			/* Getting original packet information */
			memcpy(&ssrc, &buf[4], 4);
			memcpy(&msw, &buf[8], 4);
			memcpy(&lsw, &buf[12], 4);
			
			msw = ntohl(msw) - initial_time_unix;
			lsw = ntohl(lsw);
			miliseconds = (unsigned long long)ntohl(lsw)*1000 >> 32; 

			if ((msw < current_time + 60) && (msw > current_time - 60)) {
				session->rtcp_arrived = TRUE;
				JANUS_LOG(LOG_INFO, "TimeDelta: %" PRId64 "\n", session->timedelta);
				td_seconds = session->timedelta / 1000;
				td_miliseconds = session->timedelta - (td_seconds*1000);
				JANUS_LOG(LOG_INFO, "TimeDelta seconds: %" PRId64 "\n", td_seconds);
				JANUS_LOG(LOG_INFO, "TimeDelta ms: %" PRId64 "\n", td_miliseconds);
				JANUS_LOG(LOG_INFO, "Arrived s: %" PRIu32 "\n", msw);
				JANUS_LOG(LOG_INFO, "Arrived ms: %d \n", miliseconds);
				session->first_timestamp_seconds = msw + td_seconds;
				miliseconds += td_miliseconds;
				if (miliseconds < 0){
					session->first_timestamp_seconds -= 1;
					session->first_timestamp_ms = 1000 + miliseconds;
				} else if (miliseconds > 1000) {
					while (miliseconds > 1000){
						session->first_timestamp_seconds = session->first_timestamp_seconds + 1;
						miliseconds -= 1000;
					}
					session->first_timestamp_ms = miliseconds;
				} else {
					session->first_timestamp_ms = miliseconds;
				}
				JANUS_LOG(LOG_INFO, "Seconds fixed: %d \n", session->first_timestamp_seconds);
				JANUS_LOG(LOG_INFO, "MS fixed: %d \n", session->first_timestamp_ms);

				session->first_timestamp_seconds -= seconds_since_first_rtp;
				miliseconds = session->first_timestamp_ms - ms_since_first_rtp;
				if (miliseconds < 0){
					session->first_timestamp_seconds -= 1;
					session->first_timestamp_ms = 1000 + miliseconds;
				} else {
				...
				JANUS_LOG(LOG_INFO, "Arrived timestamp: %" PRIu32 "\n", msw);
				JANUS_LOG(LOG_INFO, "First timestamp: %" PRIu64 "\n",session->first_timestamp_seconds);
				JANUS_LOG(LOG_INFO, "First timestamp ms: %" PRIu64 "\n",session->first_timestamp_ms);

				char ts_filename[256];
				strcpy(ts_filename, session->vrc->dir);
				strcat(ts_filename, "/ts.txt");
				FILE *f = fopen(ts_filename, "w");
				if (f != NULL){
					fprintf(f, "%" PRIu64 ".%d", session->first_timestamp_seconds, session->first_timestamp_ms);
					fclose(f);
				}
			}
		}
...
static void janus_echotest_hangup_media_internal(janus_plugin_session *handle) {
	...
	if(session->vrc) {
		...
		convert_to_webm(session);
		convert_to_dash(session);
...
void convert_to_webm(janus_echotest_session *session){

	FILE *fp;
	char buf[256];
	char command[256];

	snprintf(command, sizeof(command), "/opt/janus/bin/janus-pp-rec %s/video.mjr %s/video.webm", session->vrc->dir, session->vrc->dir);
	fp = popen(command, "r");
	if (fp == NULL){
		printf("Failed to post process video.");
	} else {
		printf("Executing video post-processing. \n");
...
void convert_to_dash(janus_echotest_session *session){
	...
	snprintf(folder, sizeof(folder), "%s/dasher-output", session->vrc->dir);
	mkdir(folder, 0700);
	snprintf(command, sizeof(command), "python /home/adrian/development/git/media/rumba/rumba/dasher-basic/code/Convert2DASH_unix.py %s/video.webm %s/dasher-output", session->vrc->dir, session->vrc->dir);
	fp = popen(command, "r");
	if (fp == NULL){
		printf("Failed to post process video.");
	} else {
		printf("Dashing video. \n");
		while (fgets(buf, sizeof(buf), fp) != 0) {
			printf("%s", buf);
		}
	}
...
void create_thumbs(janus_echotest_session *session){
	...
	snprintf(folder, sizeof(folder), "%s/thumbs", session->vrc->dir);
	mkdir(folder, 0700);
	snprintf(command, sizeof(command), "ffmpeg -i %s/dasher-output/video.mp4 -vf fps=1 %s/out%%d.jpg", session->vrc->dir, folder);

	fp = popen(command, "r");
	if (fp == NULL){
		JANUS_LOG(LOG_ERR, "Failed to create thumbs of video \n");
	} else {
		JANUS_LOG(LOG_ERR, "Creating thumbs of video \n");
		while (fgets(buf, sizeof(buf), fp) != 0) {
			printf("%s", buf);
		}
	}
...
static void *janus_echotest_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		// timedelta
		json_t *timedelta = json_object_get(root, "timedelta");
		if (timedelta) {
			session->timedelta = json_integer_value(timedelta);
			JANUS_LOG(LOG_INFO, "Setting timedelta property: %d \n", session->timedelta);
		}
		/* Enforce request */
		if(audio) {
		...
		if(!msg_sdp) {
			...
			if(session->vcodec == NULL){
				JANUS_LOG(LOG_WARN, "No video codec.\n");
				session->has_video = FALSE;
			}
			if(session->vcodec != NULL){
				JANUS_LOG(LOG_INFO, "Video codec found: %s.\n", session->vcodec);
			}
